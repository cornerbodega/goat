...
countryApp.controller('RoomsController', ['$scope', '$http', '$filter', '$q', 'RoomService', 'ConnectFactory', function(scope, http, filter, q, rq, cf) {
    ...
    scope.modifyRoom = function(){
        ...
        if (scope.selected === false){
            ...
            if (typeof scope.newModifyRoomName === "undefined" || scope.newModifyRoomName.length < 1){
                scope.actionStatus = "Please enter a name for your new room."
            } 
            else {
                // We know the user has entered a name for the new room
                // Is this a plant room or an inventory room?
                if (plantRoomSelected()){
                    // this is a plant room
                    // get modify plant room JSON/object
                    var modifyRoomObj = rq.modifyPlantRoomObject(scope.session,scope.selected.roomid,scope.location,scope.newModifyRoomName)
                } // end this is a plant room
                else {
                    var modifyRoomObj = rq.modifyInventoryRoomObject(scope.session,scope.selected.roomid,scope.location,scope.newModifyRoomName)
                    // this in an inventory room
                    // get modify inventory room JSON/object
                }; // end this is an inventory room

                // API call and view refresh
                cf.post(modifyRoomObj, function(modifyRoomResponse){
                    if (modifyRoomResponse.success === 1){
                        scope.actionStatus = "Success! " + scope.selected.name + " renamed to " + scope.newRoomName;
                        loadRooms();    
                    }
                    else{
                        scope.actionStatus = "Error: " + modifyRoomResponse.error;
                        console.log(modifyRoomResponse);
                    };
                });
                console.log(modifyRoomObj);
            }; // end we know the user entered a name
        }; // end a room is selected
    }; // end modify room
